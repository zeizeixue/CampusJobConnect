<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item v-if="type!='info'" class="input"  label="标题" prop="biaoti">
          <el-input v-model="ruleForm.biaoti" 
              :readonly="ro.biaoti" clearable  placeholder="标题"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item v-if="type!='info'" class="select"  label="参加人数" prop="canjiarenshu">
          <el-select v-model="ruleForm.canjiarenshu" :disabled="ro.canjiarenshu" placeholder="请选择参加人数">
            <el-option
                v-for="(item,index) in canjiarenshuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item v-if="type!='info'" class="input"  label="举办时间" prop="jubanshijian">
          <el-input v-model="ruleForm.jubanshijian" 
              :readonly="ro.jubanshijian" clearable  placeholder="举办时间"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="举办时间" prop="jubanshijian">
              <el-input v-model="ruleForm.jubanshijian" 
                placeholder="举办时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="type!='info'" class="date" label="报名时间" prop="baomingshijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.baomingshijian" class="input" label="报名时间" prop="baomingshijian">
  ...
  data() {
	...
	crossrefid : false,
      ...
      rules: {
          ...
          canjiarenshu: [
                { required: true, message: '参加人数不能为空', trigger: 'blur' },
      ...
      this.$http({
		...
		if(json.xueshengxingming!=''&&json.xueshengxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.canjiarenshuOptions = "1".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xueshengbaoming/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      objcross.canjiarenshu = parseFloat(objcross.canjiarenshu) + parseFloat(this.ruleForm.canjiarenshu)
                }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengbaoming/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengbaomingCrossAddOrUpdateFlag = false;
						 this.parent.search();
