  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row  :gutter="20" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}" class="slt">
                <el-form-item :label="contents.inputTitle == 1 ? '学生姓名' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" v-model="searchForm.xueshengxingming" clearable placeholder="学生姓名" prefix-icon="el-icon-search"></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" v-model="searchForm.xueshengxingming" clearable placeholder="学生姓名" suffix-icon="el-icon-search"></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.xueshengxingming" clearable placeholder="学生姓名"></el-input>
                </el-form-item>
        ...
        <el-row :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}" class="ad">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('kaoyanxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              icon="el-icon-s-data"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('kaoyanxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('kaoyanxinxi','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
      ...
      <div class="table-content">
        <el-table v-if="isAuth('kaoyanxinxi','查看')" v-loading="dataListLoading" :border="contents.tableBorder"
		    ...
		    prop="tijiaoshijian">
		     <template slot-scope="scope">
                  ...
                  <el-table-column :align="contents.tableAlign" :header-align="contents.tableAlign"  :sortable="contents.tableSortable"
                   label="证明文件"
                    prop="zhengmingwenjian">
                    <template slot-scope="scope">
                      <el-button size="small" type="text" @click="download($base.url+scope.row.zhengmingwenjian)">下载</el-button>
                    </template>
                  </el-table-column>
                <el-table-column  :align="contents.tableAlign" :header-align="contents.tableAlign"
                    :sortable="contents.tableSortable"
		    ...
		    prop="xuexiao">
		     <template slot-scope="scope">
              ...
              <el-table-column v-if="isAuth('kaoyanxinxi','审核')" :align="contents.tableAlign"
                  :header-align="contents.tableAlign"
                  :sortable="contents.tableSortable"
                 label="审核"
                  prop="sfsh">
            ...
            <el-table-column :align="contents.tableAlign" :header-align="contents.tableAlign"
               label="操作"
                width="300">
                <template slot-scope="scope">
                <el-button v-if="isAuth('kaoyanxinxi','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-tickets" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if="isAuth('kaoyanxinxi','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('kaoyanxinxi','查看') && contents.tableBtnIcon == 0" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if=" isAuth('kaoyanxinxi','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-edit" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                <el-button v-if=" isAuth('kaoyanxinxi','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}<i class="el-icon-edit el-icon--right" /></el-button>
                <el-button v-if=" isAuth('kaoyanxinxi','修改') && contents.tableBtnIcon == 0" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>




                <el-button v-if="isAuth('kaoyanxinxi','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-delete" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
                <el-button v-if="isAuth('kaoyanxinxi','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
                <el-button v-if="isAuth('kaoyanxinxi','删除') && contents.tableBtnIcon == 0" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
    ...
    <el-dialog
      ...
      title="统计报表"
      width="800">
        <div id="xuexiaoChart" style="width:100%;height:600px;"></div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        var xuexiaoChart = this.$echarts.init(document.getElementById("xuexiaoChart"),'macarons');
        this.$http({
            url: "kaoyanxinxi/group/xuexiao",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          zhengmingwenjian: row.zhengmingwenjian,
          xuexiao: row.xuexiao,
          sfsh: row.sfsh,
          shhf: row.shhf,
          id: row.id
