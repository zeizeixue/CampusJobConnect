<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row  :gutter="20" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}" class="slt">
              ...
              <el-form-item :label="contents.inputTitle == 1 ? '报名时间' : ''">
                  <el-date-picker
                      v-model="searchForm.baomingshijianstart"
			...
			type="datetime"
                  ...
                  至
                  <el-date-picker
                      v-model="searchForm.baomingshijianend"
                      format="yyyy-MM-dd HH:mm:ss"
			placeholder="报名时间结束"
			type="datetime"
                      value-format="yyyy-MM-dd HH:mm:ss">
        ...
        <el-row :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}" class="ad">
          <el-form-item>
            <el-button
              v-if="isAuth('qiyebaoming','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              icon="el-icon-plus"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}</el-button>
            <el-button
              v-if="isAuth('qiyebaoming','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('qiyebaoming','新增') && contents.btnAdAllIcon == 0"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}</el-button>
            <el-button
              v-if="isAuth('qiyebaoming','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
            ...
            <el-button
              v-if="isAuth('qiyebaoming','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('qiyebaoming','删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'删除':'' }}</el-button>
            ...
            <el-button
              v-if="isAuth('qiyebaoming','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              icon="el-icon-s-data"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('qiyebaoming','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('qiyebaoming','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
      ...
      <div class="table-content">
        <el-table v-if="isAuth('qiyebaoming','查看')" v-loading="dataListLoading" :border="contents.tableBorder"
		    ...
		    prop="qiyemingcheng">
		     <template slot-scope="scope">
              ...
              <el-table-column v-if="isAuth('qiyebaoming','审核')" :align="contents.tableAlign"
                  :header-align="contents.tableAlign"
                  :sortable="contents.tableSortable"
                 label="审核"
                  prop="sfsh">
            ...
            <el-table-column :align="contents.tableAlign" :header-align="contents.tableAlign"
               label="操作"
                width="300">
                <template slot-scope="scope">
                <el-button v-if="isAuth('qiyebaoming','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-tickets" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if="isAuth('qiyebaoming','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('qiyebaoming','查看') && contents.tableBtnIcon == 0" size="mini" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if=" isAuth('qiyebaoming','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-edit" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                <el-button v-if=" isAuth('qiyebaoming','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}<i class="el-icon-edit el-icon--right" /></el-button>
                <el-button v-if=" isAuth('qiyebaoming','修改') && contents.tableBtnIcon == 0" size="mini" type="primary" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>




                <el-button v-if="isAuth('qiyebaoming','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" icon="el-icon-delete" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
                <el-button v-if="isAuth('qiyebaoming','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
                <el-button v-if="isAuth('qiyebaoming','删除') && contents.tableBtnIcon == 0" size="mini" type="danger" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
    ...
    <el-dialog
      ...
      title="统计报表"
      width="800">
        <div id="canjiarenshuChart" style="width:100%;height:600px;"></div>
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        // biaoti biaoti
        //  canjiarenshu
        // xcolumn ycolumn biaoti canjiarenshu
        var canjiarenshuChart = this.$echarts.init(document.getElementById("canjiarenshuChart"),'macarons');
        this.$http({
            url: `qiyebaoming/value/biaoti/canjiarenshu`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    // 使用刚指定的配置项和数据显示图表。
                    canjiarenshuChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        canjiarenshuChart.resize();
                    };
                }
            }
        });
    ...
    getDataList() {
      ...
      let params = {
        ...
        sort: 'id',
          ...
          if(this.searchForm.baomingshijianstart!='' && this.searchForm.baomingshijianstart!=undefined ){
            params['baomingshijianstart'] = this.searchForm.baomingshijianstart
          }
          if(this.searchForm.baomingshijianend!='' && this.searchForm.baomingshijianend!=undefined){
            params['baomingshijianend'] = this.searchForm.baomingshijianend
          }
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
            params['sfsh'] = this.searchForm.sfsh
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          canjiarenshu: row.canjiarenshu,
          jubanshijian: row.jubanshijian,
